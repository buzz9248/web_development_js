...
```
이상의 코드를 실행했을 때, 부드럽게 전환되지 않고 깜빡거리면서 전환되는 것을 확인할 수 있습니다.
...
```
.fadeOut()에 두 개의 argument를 삽입. # 1의 경우 400ms로 지정했습니다 -> callback function이 두 번째 argument로 지정되어있기 대문에 시간 지정이 default로 이루어져야만 합니다.

그리고 두 번째 argument는 본래 함수인 fadeOut()이 전부 실행된 이후에 .html()과 .fadeIn()이 실행도도록 두 개의 실행문을 하나의 익명함수로 묶은 것을 의미함.
...
$(`#text`).fadeOut(400, function() {
  ...
  console.log(4);
```
이상의 코드 해석: 맨 처음에 1이 찍히고 fadeOut()이 실행된 후에 4가 찍혔습니. (callback function 제어권 바깥이기 때문에). 그리고 0.4초 후에 2, 3이 찍혔습니다.
...
zip파일 받아서 필요한 파일을 script 폴더에 넣었습니다.

-> index.html 에서 \<script> 태그 내에 집어넣어줘야함.

-> 중요한 것은 animateNumber가 제이쿼리의 플러그인이기 때문에 제이쿼리가 꼭 먼저 선행되어야 된다는 점을 명심해야함.

-> 또한, 저희가 작성한 script2.js 역시도 해당 플러그인을 사용한다면 제이쿼리 플러그인이 선행되어야 합니다.
...
- Attribute: HTML 태그 내에 정의된 값(DOM이 로드될 때 처음 설명된 값)
