...
function renderTodos(todos) {
  ...
  todos.forEach((todo, index) => {
    ...
    // 기능과 관련된 부분이 없습니다 -> 즉 renderTodos() 내부에 button의 삭제 기능을 구현해둬야 합니다.
    ...
    // deleteBtn.addEventListener('click', removeTodo(index)); 와 동일함
...
function removeTodo(index) {
  ...
  // '배열에서만 삭제'가 의미하는 것 -> 로컬 스토리지에는 남아있다 -> 그렇다면 다시 읽어오게 된다면 
  // 삭제버튼 눌러서 배열에서 사라진 애가 어차피 다시 나옴
  localStorage.setItem('todos', JSON.stringify(todos));
  renderTodos(todos);
}